...
function encryptData(textE, keyE){

    if (textE.length === 0 || keyE.length === 0) {
      return;
    }
    if (textE.length <=10000 && keyE.length <=10000) {
      return Base64.encode(AES.encrypt(ASCII.decode(textE), ASCII.decode(keyE)));
    }else{
      return 0;
    }
    }
function decryptData(textD, keyD){
    if (textD.length === 0 || keyD.length === 0) {
      return;
    }
    if (textD.length <=10000 && keyD.length <=10000) {
    var textDD = Base64.decode(textD);
    if (textDD === false) {
      return;
    } else {
   return ASCII.encode(AES.decrypt(textDD, ASCII.decode(keyD)));
